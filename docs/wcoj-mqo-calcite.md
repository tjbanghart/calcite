...
**Sorted trie approaches.** Veldhuizen's *Leapfrog Triejoin* [4] (ICDT 2014) proved that WCOJ is practical, but requires pre-sorted tries — a constraint that limits deployment to systems like LogicBlox where data arrives pre-sorted. Our hash-based approach eliminates this requirement, enabling WCOJ on general SQL inputs without index pre-computation.

**Graph-specialized engines.** EmptyHeaded [14] (TODS 2017) achieves order-of-magnitude speedups over binary-join engines on graph pattern queries using SIMD-accelerated sorted-trie intersection and a GHD-based variable ordering compiler. But it inherits the pre-sorted index requirement and does not support general SQL workloads — both gaps our system addresses.

**Hash-based integration into general-purpose RDBMS.** Freitag et al. [5] (PVLDB 2020) showed that hash-based WCOJ can be built inside a general-purpose RDBMS, using tries constructed at query execution time with no pre-sorting. Their Umbra integration is the closest precursor to ours — we adopt the same hash-trie approach — but their system processes each query independently. The multi-query dimension is entirely absent.

**Unified approaches.** Free Join [15] (SIGMOD 2023) unifies binary and WCOJ under a single framework using a *free join trie* that generalizes both hash tables and sorted tries. Like Umbra, it optimizes individual queries without considering cross-query sharing.
...
**The selection problem.** The general MQO problem — choosing which sub-expressions to materialize — is NP-hard [7]. Kathuria and Sudarshan [7] achieved a $(1 - 1/e)$ approximation by reformulating it as submodular maximization. Our work does not address this selection problem; we instead exploit the specific structure of WCOJ computation to eliminate redundancies within batches of cyclic queries without a combinatorial selection step. Jindal et al. [25] (VLDB 2018) showed the selection problem's importance at datacenter scale, where tens of thousands of jobs exhibit significant computation overlap. Zinchenko and Ponomaryov [10] survey the broader selection landscape.

**Operator-based approaches most relevant to this work.** Roy et al. [8] (SIGMOD 2000) showed MQO can be implemented as a lightweight Volcano extension — but only for binary-join plans. Tu et al. [11] (IEEE BigData 2022) proposed $\psi$-operators that algebraically combine multiple queries into a single expression, achieving up to 36x speedup. Our `Combine` operator is similar in spirit but is designed to compose with WCOJ execution rather than binary joins. Harizopoulos et al. [22] (SIGMOD 2005) introduced QPipe, which shares work across queries dynamically at runtime; our approach identifies sharing at compile time, making it less adaptive but cheaper to execute. In industry, Bruno et al. [26] (ICDE 2022) handle common expressions in Amazon Athena without materialization (via operator fusion), while Roy et al. [27] (VLDB 2021) implemented workload-level computation reuse at scale in Microsoft's SparkCruise; both systems operate exclusively on binary-join plans.
...
┌──────────────────────────────────────────────────────┐
│  Parser (Parser.jj)                                  │
│  Recognizes MULTI() → SqlCall(MULTI)                 │
└──────────────────────┬───────────────────────────────┘
                       │
                       ▼
┌──────────────────────────────────────────────────────┐
│  SQL-to-Rel (SqlToRelConverter.java)                 │
│  Each sub-query → RelNode; all wrapped in Combine    │
└──────────────────────┬───────────────────────────────┘
                       │
                       ▼
┌──────────────────────────────────────────────────────┐
│  Planner (VolcanoPlanner)                            │
│  ├─ EnumerableWCOJRule: MultiJoin → EnumerableWCOJ   │
│  ├─ ··· CombineSharedComponentsRule [Share only]     │
│  └─ ··· EnumerableCombineWCOJPrefixRule [Share only] │
└──────────────────────┬───────────────────────────────┘
                       │
                       ▼
┌──────────────────────────────────────────────────────┐
│  Code Generation (EnumerableCombine.implement())     │
│  Creates TrieCache ──────────────────────────────┐   │
│  Visits each child EnumerableWCOJ, passing cache  │   │
└──────────────────────┬──────────────────────────────┘│
                       │                               │
                       ▼                               │ (shared object)
┌──────────────────────────────────────────────────────┤
│  Runtime (WCOJEnumerator × N operators)            ←─┘
│  Each operator calls TrieCache.getOrBuild()          │
│  → first call builds HashTrie; subsequent calls hit  │
│  → cache; backtracking search over shared tries      │
└──────────────────────────────────────────────────────┘
```
Dashed rules (···) indicate Combine-Share-only components.

The dashed components (CombineSharedComponentsRule, EnumerableCombineWCOJPrefixRule) are opt-in planner rules active only in Combine-Share mode. All other components are active whenever `MULTI()` is used. A complete mapping of components to source files is provided in Appendix A.
...
Detecting cyclic join patterns requires two sequential steps: (1) flattening the binary join tree into a single multi-way node so that all join predicates are visible simultaneously, and (2) testing whether the resulting join graph contains a cycle.

**Step 1: Flattening via `JoinToMultiJoinRule`.** Calcite's standard planner represents joins as a binary tree of `LogicalJoin` nodes. Individual pairwise joins carry only their local condition (e.g., $R.b = S.b$), so cyclicity is not detectable at any single node in the tree. `JoinToMultiJoinRule` (a standard Calcite rule) collapses an inner-join subtree into a single `MultiJoin` node with $N$ inputs and a single combined condition formed by conjoining all join predicates. Once this flattening has occurred, all equi-join conditions are co-located, making the global join structure accessible for analysis. `EnumerableWCOJRule` matches specifically on `MultiJoin` nodes, making `JoinToMultiJoinRule` a required prerequisite in the planner configuration.

**Step 2: Join hypergraph construction.** From the `MultiJoin.joinFilter` — a conjunction of equi-join predicates over globally-indexed fields — we build the join hypergraph:

1. **Field offset computation.** Each input $i$ occupies a contiguous range $[\text{offset}_i, \text{offset}_{i+1})$ of global field indices, computed by accumulating row-type widths.
2. **Equi-join extraction.** We traverse the condition recursively, collecting `EQUALS(RexInputRef, RexInputRef)` atoms into a list of global field-index pairs.
3. **Equivalence class construction.** A Union-Find structure (with path compression and union-by-rank) groups all globally co-equated fields into equivalence classes. Each class is a *join variable* — a column shared across one or more inputs.
4. **Hyperedge projection.** Each equivalence class that spans at least two distinct inputs becomes a `JoinVariable`. The join *hypergraph* has inputs as nodes and `JoinVariable`s as hyperedges. Because all join predicates in our benchmarks are binary (linking exactly two inputs), the hyperedges are all of degree 2 and the hypergraph reduces to a standard undirected graph.

**Step 3: Cyclicity test.** We project the hypergraph onto a simple undirected graph where nodes are inputs and edges connect pairs of inputs sharing a `JoinVariable`. A connected graph with $|E| \geq |V|$ contains at least one cycle (a spanning tree on $|V|$ nodes uses exactly $|V| - 1$ edges; any additional edge creates a cycle). The connected-graph assumption holds for any meaningful multi-way join — a disconnected join graph would imply a Cartesian product between independent sub-queries. This test fires `EnumerableWCOJRule` and suppresses it for acyclic queries (chains, stars, snowflakes) where binary join ordering is already optimal.

**Relationship to `DpHyp`.** Calcite already includes `HyperGraph` — a first-class `RelNode` representing a join hypergraph — and `DpHyp` [28] (`@Experimental`), Moerkotte and Neumann's DP algorithm that operates on it, enumerating connected subgraph / complement (csg-cmp) pairs to find the minimum-cost binary join tree. Our `analyzeJoinGraph` method in `EnumerableWCOJRule` builds a parallel but separate `JoinGraph` using its own internal `UnionFind`, rather than reusing the existing `HyperGraph` infrastructure. The purpose diverges: `DpHyp` answers *in what order should these binary joins execute?*, while `EnumerableWCOJRule` answers *should these joins be executed as WCOJ at all?* Once the cyclicity test fires, the join ordering question is sidestepped entirely — WCOJ processes all variables simultaneously in a fixed global ordering, eliminating the csg-cmp enumeration `DpHyp` performs. A natural future refactoring would unify the two constructions and allow a hybrid strategy: `DpHyp` for acyclic sub-plans, WCOJ for cyclic cores (see Section 9, future work item 4).
...
**Important:** `TrieCache` sharing has two distinct scopes, and the difference directly explains the experimental gap between Combine and Combine-Share modes:

1. **Intra-operator sharing** is always active. Within a single WCOJ operator, self-joins reference the same input `Enumerable` multiple times. The `TrieCache` builds one trie per `(input, keyIndex)` pair, avoiding redundant construction within a query.

2. **Cross-operator sharing requires spooling.** Without `CombineSharedComponentsRule`, sibling WCOJ operators within a `Combine` independently generate code for their own table scans, producing *separate* Java object references. `TrieCache` will not find a cache hit across operators in this case. Cross-operator sharing requires `Combine-Share` mode (Section 5.2), which spools shared sub-expressions so multiple operators read from the same materialized `Enumerable` object.

The design is deliberately simple:

- **Identity-based lookup.** Uses `IdentityHashMap` so sharing occurs only when inputs are the *same Java object reference*. Structural equality would require hashing the contents of an `Enumerable`, which may be expensive or impossible for streaming inputs.
...
The rule skips leaf table scans (handled by trie caching) and the `Combine` root, focusing on higher-level sub-trees where materialization benefit is greatest.
...
**Theorem 1.** *Let $Q_1$ and $Q_2$ be two WCOJ queries with variable orderings $(x_1, \ldots, x_m)$ and $(x_1, \ldots, x_K, y_{K+1}, \ldots, y_{m'})$ respectively, such that (a) variables $x_1, \ldots, x_K$ have identical fingerprints, and (b) the corresponding inputs are shared via `CombineSharedComponentsRule` so that `TrieCache` returns identical trie objects for corresponding variables. Then the prefix bindings $\{(v_1, \ldots, v_K)\}$ computed by $Q_1$'s prefix enumerator are exactly the prefix bindings that $Q_2$'s full enumerator would produce for its first $K$ variables.*
...
**Analysis.** Per-query amortized cost for `Combine` drops from 105 ms at $N=2$ to **11 ms** at $N=20$, a 9.5x reduction. This strong sub-linear scaling demonstrates that the `Combine` operator successfully amortizes fixed costs (plan compilation, trie construction) across the batch.
...
Combine-Share achieves the best speedup at $N=10$ (3.4x over baseline, vs. 3.0x for plain Combine), confirming that prefix sharing provides a real benefit when duplicate sub-queries exist (at $N=10$, each of the 5 query variations appears twice). The $N=10$ baseline has high variance (CV = 31.5%, marked $^\dagger$), so these ratios carry ~20% uncertainty — at the lower end of the baseline's range (~400 ms), the speedups would be closer to 2.1x and 2.4x. The per-query cost of 17 ms for Combine-Share vs. 19 ms for Combine reflects the savings from computing the shared WCOJ prefix once per pair of identical sub-queries.
...
*WCOJ benefits depend on join structure.* The self-join queries demonstrate clear WCOJ benefits: the 4-cycle achieves **3.1x** speedup and the triangle **2.0x**, driven by intermediate result explosion in binary join plans.

**A note on TPC-H variance.** The triangle and 4-cycle baseline measurements exhibit high variance — CV = 22% and 24% respectively, marked $^\dagger$. Unlike short synthetic queries where JIT compilation is the primary variance source, these multi-second queries are affected by JVM GC pressure and OS page cache state. The 4-cycle baseline's first iteration (213s vs. ~120s steady-state) reflects OS page cache cold-start that persists through warmup when four copies of lineitem are scanned under different join orderings. We retain this outlier because it reflects a realistic cold-start scenario; eliminating it would require additional warmup rounds each taking ~40 seconds. Consequently, all TPC-H speedup ratios should be interpreted as estimates with ~20% uncertainty.

The 4-cycle is particularly telling: binary joins with `MULTI()` batching run out of memory at 4 GB heap (marked OOM in Table 5), while WCOJ completes in 41 seconds — demonstrating that WCOJ's advantage on deep cyclic queries is not merely a speedup but a qualitative difference in feasibility. However, WCOJ is not universally beneficial. The FK rectangle and diamond queries (Table 6) show WCOJ performing 1.5x–3.6x *slower* than binary joins. The critical difference is join selectivity: the FK queries close their cycles through `nationkey` (25 distinct values), creating a highly selective closing predicate that binary hash joins handle efficiently. WCOJ's variable-at-a-time enumeration explores all candidates at each level, paying $O(\text{fan-out at level } k)$ intersection cost per variable even when most candidates are eliminated — a cost that does not appear in the simplified cost model of Section 3.5 but dominates when fan-out is high and final selectivity is low. The self-join queries, by contrast, close their cycles through `partkey` or `orderkey` with high final cardinality (300K–6M rows), making WCOJ's avoidance of intermediate materialization the dominant effect.
